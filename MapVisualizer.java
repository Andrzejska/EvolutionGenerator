...
public class MapVisualizer {
    ...
    private String drawObject(Vector2d currentPosition) {
        String result = null;
        if (this.map.isOccupiedMapVisual(currentPosition)) {
            List<Object> object = (List<Object>) this.map.objectAt(currentPosition);
            if (object != null) {
               if(object.size()==1) return object.get(0).toString();
               else{
                   int numberOfElements=0;
                   for(Object o:object){
                       numberOfElements++;
                   }
                   return String.valueOf(numberOfElements);
               }
            } else {
                result = EMPTY_CELL;
            }
